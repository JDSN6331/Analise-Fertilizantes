(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))r(t);new MutationObserver(t=>{for(const e of t)if(e.type==="childList")for(const i of e.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&r(i)}).observe(document,{childList:!0,subtree:!0});function o(t){const e={};return t.integrity&&(e.integrity=t.integrity),t.referrerPolicy&&(e.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?e.credentials="include":t.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function r(t){if(t.ep)return;t.ep=!0;const e=o(t);fetch(t.href,e)}})();let n=null,c="todos",d="volume",m="";const p=document.getElementById("loader"),g=document.getElementById("week-select-global"),f=document.getElementById("branch-search-input");async function w(){try{const s=await fetch("./data/dashboard_data.json");if(!s.ok)throw new Error("Falha ao carregar o arquivo JSON de dados");n=await s.json(),p&&(p.style.display="none"),n&&(x(),v())}catch(s){console.error("Erro na inicialização do painel:",s),p&&(p.innerHTML=`
  ...
  `}function $(){if(!n)return;const s=document.getElementById("legend-regions-grid");if(!s)return;const a=[...n.mesorregionais].sort((o,r)=>o.name.localeCompare(r.name));s.innerHTML=a.map(o=>{const r=o.filiais||[],t=r.map(e=>{const i=e.name.replace("Loja ","").replace("Unidade Avançada ","UA ");return`<span class="legend-store-chip" title="${e.name}">${i}</span>`}).join("");return`
      <div class="legend-region-card">
        <div class="legend-region-header">
          <span class="legend-region-title">${o.name}</span>
          <span class="legend-store-count">${r.length} Lojas</span>
        </div>
        <div class="legend-stores-list">
          ${t}
        </div>
      </div>
